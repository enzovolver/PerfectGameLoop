...
public class Game extends Canvas implements Runnable{ // Cria a Classe GameLoopingProfessional que extende a classe Canvas e Implementa a Interface Runnable
    ...
    public void janela() {
        ...
        this.setPreferredSize(new Dimension(WIDTH * SCALE, HEIGHT * SCALE)); // Utiliza a classe Dimension para dimensionar a janela
    ...
    public void render() {
        ...
        //g2.rotate(Math.toRadians(45),  10 + (8 * SCALE), 10 + (8 * SCALE));
        //g2.drawImage(this.getP().animation(), 10, 10, 16 * SCALE, 16 * SCALE, null); // Coloca o personagem
        //g2.rotate(Math.toRadians(-45), 10 + (8 * SCALE), 10 + (8 * SCALE));

        g.dispose(); // Método de otimização
        g = bs.getDrawGraphics(); // ???
        g.drawImage(this.getImage(), 0, 0, SCALE * WIDTH, SCALE * HEIGHT, null); // ???
